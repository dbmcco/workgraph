...
pub fn opencode_model_arg(model: &str) -> Option<String> {
    ...
    let model_arg = if matches!(spec.provider.as_deref(), Some("zai" | "z-ai")) {
        format!("zai/{}", spec.model_id)
    } else if provider == Some("openrouter") {
        let id = spec
            .model_id
...
mod tests {
    ...
    #[test]
    fn opencode_model_arg_accepts_zai_provider_spelling() {
        assert_eq!(
            opencode_model_arg("zai:glm-5.1"),
            Some("zai/glm-5.1".to_string())
        );
        assert_eq!(
            opencode_model_arg("z-ai:glm-5.1"),
            Some("zai/glm-5.1".to_string())
        );
        assert_eq!(
            opencode_model_arg("opencode:zai/glm-5.1"),
            Some("zai/glm-5.1".to_string())
        );
    }
