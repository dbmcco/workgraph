...
fn resolve_endpoint_execution_config(
    ...
    let model_provider = effective_model
        .and_then(|model| worksgood::config::parse_model_spec(model).provider)
        .map(|provider| worksgood::config::provider_to_native_provider(&provider).to_string());
    ...
    let provider_env_vars = endpoint
        .map(|ep| {
            worksgood::config::EndpointConfig::env_var_names_for_provider(&ep.provider)
                .iter()
