...
defaults cover the common case. To commit choices to disk run `wg config init
--global` (minimal canonical codex-cli config; pass `--route claude-cli` /
`codex-cli` / `openrouter` / `local` / `nex-custom` for non-default routes) or
`wg setup` (interactive wizard). To inspect a config
...
pinned to `claude:haiku` running on the claude CLI — still available as an
explicit worker route — and ignore project-level provider cascade from
